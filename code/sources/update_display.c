...
/*   Updated: 2019/08/08 14:32:31 by mhouppin    ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
void	update_dump(void)
{
	for (size_t	y = 0; y < 32; y++)
	{
		...
		{
			g_dpixels[y * 256 + x] = g_vimcol[g_memmap.fixed_ram[y * 256 + x]];
		}
	}
	for (size_t y = 0; y < 32 * g_cart.n_ram_banks; y++)
	{
		for (size_t x = 0; x < 256; x++)
		{
			g_dpixels[8192 + y * 256 + x] = g_vimcol[g_memmap.extern_ram[y * 256 + x]];
		}
	}
	SDL_UnlockTexture(g_dtexture);
	SDL_RenderClear(g_drender);
	SDL_RenderCopy(g_drender, g_dtexture, NULL, NULL);
	SDL_RenderPresent(g_drender);
